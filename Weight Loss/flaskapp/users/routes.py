...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(animal=form.animal.data, email=form.email.data, password=hashed_password)
        db.session.add(user)
        db.session.commit()
...
def account():
    ...
    if form.validate_on_submit():
        current_user.animal = form.animal.data
        current_user.email = form.email.data
        db.session.commit()
        flash('Your account has been updated!', 'success')
        return redirect(url_for('users.account'))
    elif request.method == 'GET':
        form.animal.data = current_user.animal
        form.email.data = current_user.email
    # image_file = url_for('static', filename='profile_pics/' + current_user.image_file)
    return render_template('account.html', title='Account', form=form) # image_file=image_file


@users.route("/user/<string:animal>")
def user_posts(animal):
    page = request.args.get('page', 1, type=int)
    user = User.query.filter_by(animal=animal).first_or_404()
